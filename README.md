...
- 交付基础：FastAPI、静态 Web、Docker Compose、GHCR、Prometheus、Alertmanager、K8s manifests、Qdrant StatefulSet / Service / PVC / PDB、Qdrant CronJob manual Job smoke、Qdrant CronJob natural schedule observe、Qdrant CronJob multi-cycle observe、服务器 Docker Compose 24 小时 long-run smoke、服务器 API Down 故障可见性演练
...
本机 Docker Compose smoke 说明见 [Local Long-Run Smoke](docs/deployment/local-long-run-smoke.md)。
...
- 服务器 Docker Compose 24 小时 long-run smoke 验证
- 服务器 API Down 故障可见性演练：Prometheus firing、Alertmanager active、恢复后 resolved

仍待推进：

- 真实 Feishu / WeCom / email 通知提供方
- 数据恢复 drill 和真实通知通道验证
- 用户认证和更完整的 Trace 查看器
- Langfuse 或等价可观测平台接入
